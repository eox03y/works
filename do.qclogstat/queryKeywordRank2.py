...
def filer_for_LHRHVH_query(line):
	...
	if q==None or shop==None or svc==None: return None
	try:
...
class LogAnalyzer:
	...
	def	write_info(self):
		for SVC_CTRY,kdict in self.statD.iteritems():
			fname = "%s.%s.%s.csv" % (self.SVC, self.myname, SVC_CTRY)
			print "Save to:", fname
			N = 1500
...
if __name__=="__main__":
	#test_w_local_LHRHVH()
	#test_w_local_APPS()
	#do_many_days_s3_LHRHVH()
	do_many_days_s3_APPS()
